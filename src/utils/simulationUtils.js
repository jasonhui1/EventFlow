    ...
    for (const { node: parentNode } of parentNodes) {
        ...
        if (parentNode.type === 'referenceNode' && parentNode.data?.referenceId) {
            ...
            if (referencedEvent && referencedEvent.nodes && referencedEvent.edges) {
                // If resolving references, create a localized map for O(1) lookups
                const innerNodeMap = new Map();
                referencedEvent.nodes.forEach(n => innerNodeMap.set(n.id, n));

                const refEndNode = innerNodeMap ? Array.from(innerNodeMap.values()).find(n => n.type === 'endNode') : referencedEvent.nodes.find(n => n.type === 'endNode');
                if (refEndNode) {
...
export const getComposedPrompt = (nodeId, allEvents, nodes, edges, currentEventFixedPrompt = '', options = {}) => {
    ...
    if (options.resolveReferences !== false && node.type === 'referenceNode' && node.data?.referenceId) {
        ...
        if (refEvent && refEvent.nodes) {
            const innerNodeMap = new Map();
            refEvent.nodes.forEach(n => innerNodeMap.set(n.id, n));

            const refEndNode = innerNodeMap ? Array.from(innerNodeMap.values()).find(n => n.type === 'endNode') : refEvent.nodes.find(n => n.type === 'endNode');
            if (refEndNode) {
...
export const simulateEvent = (
    ...
    // Create node map for O(1) lookups during traversal
        ...
        if (currentNode.type === 'branchNode') {
            if (outgoingEdges.length > 0) {
                ...
                selectedEdges.forEach(edge => {
                    ...
                    const targetNode = nodeMap ? nodeMap.get(edge.target) : processedNodes.find(n => n.id === edge.target);
                    if (targetNode) queue.push(targetNode);
                });
            }
        } else if (currentNode.type === 'ifNode') {
            // Evaluate condition based on Start Node inputs
            const startNode = startNodes.length > 0 ? startNodes[0] : processedNodes.find(n => n.type === 'startNode');
